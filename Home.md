# Compbuild Studio Tools

An original, in-game **CAD-style world editor** for **Minecraft Java 1.21.11
(Fabric)** — a docked editor with a tool rail, live previews, reversible
history, and a clean monochrome/orange UI. Build fast in singleplayer, or on a
Paper server with the companion plugin.

> **Beta build — expect bugs, and more tools and fixes are still coming.**
> Not affiliated with Axiom.

---

## What it does

- **Selections & block ops** — box-select, fill, delete, replace, copy/cut/paste,
  all undoable.
- **Terrain sculpting** — twelve brush moves (Raise, Smooth, Cliff, Slope, …)
  with falloff curves and natural edges.
- **Painting** — re-skin surfaces or fill volumes with a single material,
  a weighted **noise palette**, or a **gradient**.
- **Paths** — roads, walls, drapes, smooth curves, and clipboard **stamps**
  along clicked points, with a live textured preview.
- **Geometry** — exact shapes (lines, circles, spheres, domes, cylinders…)
  from a few points.
- **Transform** — rotate a clipboard to *any* angle, scale per axis, mirror,
  grid-snap, and place a textured ghost.
- **Biomes** — paint biomes with the same noise patterns, undoably.
- **Schematics** — import/export `.schem` / `.schematic` through a thumbnail
  browser with named folders.
- **Materials** — searchable picker (Air included), recent materials, and
  **named palettes** you save and reuse.
- **Multiplayer** — a **Paper server plugin** applies edits with authority,
  server-side undo, permissions, and optional **WorldGuard** region respect.

---

## Start here

- **[[Installation]]** — get the mod (and, for servers, the plugin) running.
- **[[Getting Started]]** — open the editor and make your first edit.
- **[[Controls]]** — every key and click.
- **[[Tools]]** — the full tool reference.
- **[[Materials, Palettes & Presets]]** — materials, Air, recents, palettes, presets.
- **[[Schematics]]** — import/export builds.
- **[[Multiplayer (Paper)]]** — server setup, WorldGuard, config, troubleshooting.
- **[[Top Bar & Settings]]** — the File/Edit/Settings/Help menus.
- **[[FAQ]]** — common questions.

---

## Requirements

| | |
| --- | --- |
| Minecraft | 1.21.11 (Java Edition) |
| Loader | Fabric Loader ≥ 0.19.3 + Fabric API |
| Java | 21+ |
| Server (optional) | Paper/Spigot 1.21.x for multiplayer |
